...
Vue.component("draglinechart", {
    ...
    data: function () {
        return {
            ...
            prevCategory: "single",//上一次分类
            dradRange: [0, 0],//框选范围
            tableModal: false,
            history: {},//操作历史，记录拖拽结束回调时的diff数据,用于撤销操作
            historyIndex: 0,//历史记录指针
            historyList: [],//历史列表
            DRAG_ZLEVEL: 2,
            DRAG_Z: 11,
            DRAG_POINT_COLOR: '#00FF00',
            SEL_LINE_COLOR: '#fff',
            HIDDEN_DRAG_POINT: false,
    ...
    computed: {
        ...
        allowSetChart: function () {
            return this.echart && this.currentDragLine;
        },
    ...
    methods: {
        ...
        initChart: function () {
            ...
            this.echart.on('dblclick', 'series.line', (params) => {
                var id = params.seriesId;
                if (this.finallyDragLine[id]) {
                    this.currentDragLine = id;
                    this.changeLine();
                }
            });

            //初始化选中曲线
            this.currentDragLine = Object.keys(this.finallyDragLine)[0];
        ...
        initZlevel: function () {
            //分为三层，从下向上的层数分别为 不会修改的层，数值修改但不会拖拽的层，可拖拽层
            var series = this.chartOption?.series || [];
            series.forEach((item, index) => {
                if (this.canDrag(item)) {
                    item.zlevel = 1;
                }
                else if (this.isSum(item)) {
                    item.zlevel = 1;
                } else {
                    item.zlevel = 0;
                }
        ...
        initHistory: function () {
            var historyItem = {
                selLine: this.currentDragLine,
                finallyDragLine: DataHelper.clone(this.finallyDragLine),
            }
        ...
        updateDraw: function (x, y) {
            ...
            requestAnimationFrame(() => {
                ...
                var l = this.finallyDragLine[this.currentDragLine].length;
                chartValues.forEach(item => {
                    if (item[0] >= 0 && item[0] < l)
        ...
        createSignalDargPoint: function () {
            ...
            var graphicListItem = data.map((item, dataIndex) => {
                return {
                    ...
                    style: {
                        fill: this.DRAG_POINT_COLOR,
                    },
                    invisible: this.HIDDEN_DRAG_POINT,
                    draggable: 'vertical',
                    ...
                    ondragend: ((key, dataIndex) => {
                        return (event) => {
                            ...
                            // 调用两次updatePosition  是因为如果坐标轴范围变化那么上一次计算出来的位置是不准的，所以需要再次计算一次
                            setTimeout(() => {
                                this.updatePosition();
                            }, 100)
        ...
        updatePosition: function () {
            if (!this.echart || !this.isEdit || !this.currentDragLine) {
                this.deleteGraphic();
                return
            };
            var option = this.echart.getOption();
        ...
        setSelLineHigh: function () {
            ...
            series.forEach((item, index) => {
                ...
                if (this.canDrag(item)) {
                    item.data = this.finallyDragLine[item.id];
                    if (item.lineStyle) {
                        delete item.lineStyle.color;
                    }
                    if (item.id == this.currentDragLine) {
                        if (item.lineStyle) {
                            item.lineStyle.color = this.SEL_LINE_COLOR
                        } else {
                            item.lineStyle = { color: this.SEL_LINE_COLOR }
                        }
                    }
        ...
        canDrag: function (item) {
            return item[this.DRAG_MARK] && item.type == 'line'
        },
        isSum: function (item) {
            return item[this.DRAG_SUM_MARK] && item.type == 'line'
        },
        ...
        recordHistory: Descorate.debounce(function () {
            var historyList = this.historyList;
            var selLine = this.currentDragLine;
            var finallyDragLine = DataHelper.clone(this.finallyDragLine);
            var historyItem = {
                selLine: selLine,
                finallyDragLine: finallyDragLine,
            }
            if (historyList.length - 1 == this.historyIndex) {
                historyList.push(historyItem);
                this.historyIndex++;
            } else {

                historyList.splice(this.historyIndex + 1);
                this.historyIndex = historyList.length - 1;
                historyList.push(historyItem);
                this.historyIndex++;
            }
        }, 300),
        undo: function () {
            ...
            var historyList = this.historyList;
            if (historyList.length > this.historyIndex && this.historyIndex > 0) {
                // 弹出当前状态
                this.historyIndex--
                // 获取上一个状态
                var prev = historyList[this.historyIndex];
                // 恢复图表和其他状态
        ...
        restoreState(stateObj) {
            this.currentDragLine = stateObj.selLine;
            this.finallyDragLine = DataHelper.clone(stateObj.finallyDragLine);
            this.changeLine();
            this.updateTable();
            this.updateChart();
            this.updateDragSum();
        ...
        contextmenuClick: function (event) {
            ...
            var input = this.$refs.contextInputRef;
    ...
    created: function () {
        this.zrMousedown = (event) => {
            if (this.zrMousedown.clickTimer) {
                clearTimeout(this.zrMousedown.clickTimer);
                this.zrMousedown.clickTimer = null;
            } else {
                this.zrMousedown.clickTimer = setTimeout(() => {
                    this.mouseBuffer = [];
        ...
        this.zrMousedown.clickTimer = undefined;
    ...
    watch: {
        ...
        dargconfig: {
            ...
            handler: function (val) {
                ...
                if (Array.isArray(this.config.categorys)) {
                    this.categoryList = all.filter(item => {
                        return this.config.categorys.includes(item.value)
                    })
    ...
    template: `
        ...
        <div style="width:100%;display:flex;justify-content:space-between;align-items:center;">
            ...
            <div v-show="isEdit" style="display:flex;margin-left:14px;">
                <buttons v-if="config.showCategory" v-model="currentCategory"   :source="categoryList" :preventrepeatclick = "false" @change="changeDrag" ></buttons>       
        ...
        <div style="flex:1;display:flex;max-height: 100%;" ref="contentWrap">
            ...
            <div  v-show="sliderTableShow" style="width:242px">
                <Table border :columns="columns" :data="tableData" :max-height="tableHeight">
                    <template slot-scope="{ row, index }" slot="finalValue">
                    <span v-if="tableReadonly">{{row.finalValue}}</span>
                    <InputNumber v-else style="width:120px;" v-model="row.finalValue"     size="small" controls-outside @on-change="changeFinallyValue(row,index)"></InputNumber>
                    </template>
        ...
        <div ref="contextmenuRef" v-show="showContextMenu" style="position:absolute;" @keyup.enter="saveDiff">
